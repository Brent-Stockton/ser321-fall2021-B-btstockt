    ...
    public void start() throws IOException {
        ...
        try {
            ...
            while(true) {
                ...
                // receives new game request and send board and initial task to client
                Request chooseHit = Request.parseDelimitedFrom(in);

                if (chooseHit.getOperationType() == Request.OperationType.NEW) {
                    game.newGame();
                    writeToLog(name, Message.START);
                    System.out.println("New Game started for: " + name);
                    Response response = Response.newBuilder()
                            .setResponseType(Response.ResponseType.TASK)
                            .setImage(game.getImage())
                            .setTask("Please choose a row and column...")
                            .build();
                    response.writeDelimitedTo(out);
                    System.out.println(game.getUnhiddenImage());
                ...
                while (gameLoop) {
                    ...
                    //chooseHit = Request.parseDelimitedFrom(in);
                    //Case 2 gameplay
                    switch (chooseHit.getOperationType()) {

                        case LEADER:

                            System.out.println("Server got leaderboard request from: " + name);
                            writeToLog(name, Message.START);
                            System.out.println("Okay " + name);
                            Response leader = Response.newBuilder()
                                    .setResponseType(Response.ResponseType.LEADER)
                                    .setMessage("Leaderboard sent")
                                    .build();
                            leader.writeDelimitedTo(out);

                            break;


                        case ROWCOL:
                            game.replaceOneCharacter(chooseHit.getRow(), chooseHit.getColumn());
                            writeToLog(name, Message.START);
                            System.out.println("Checking Board for client:  " + name);

                            if (game.isAHit(chooseHit.getRow(), chooseHit.getColumn()) == true
                                    && score < 4) {
                                score++;
                                ...
                                response = Response.newBuilder()
                                        ...
                                        .setImage(game.getImage())
                                        .setHit(true)
                                        .setMessage("Hit Detected! Your score is: " + Integer.toString(score))
                                        .setTask("Please choose a row and column...")
                                        .build();
                                System.out.println(game.getUnhiddenImage());
                                System.out.print("Client Score: " + (score));
                            } else if (game.isAHit(chooseHit.getRow(), chooseHit.getColumn()) == true
                                    && score == 4) {
                                ...
                                response = Response.newBuilder()
                                        ...
                                        .setImage(game.getImage())
                                        .setHit(true)
                                        .setMessage("Final Hit Detected! You Win!!!")
                                        .build();
                                ...
                                response = Response.newBuilder()
                                        ...
                                        .setImage(game.getImage())
                                        .setHit(false)
                                        .setMessage("Sorry you missed! Your Score is: " +
                        ...
                        case QUIT:
                            System.out.println("Server got quit request from client: " + name);
                            writeToLog(name, Message.START);
                            System.out.println("Okay " + name);
                            Response quit = Response.newBuilder()
                                    .setResponseType(Response.ResponseType.BYE)
                                    .setMessage("Server: Thanks for Playing. GoodBye!!")
                                    .build();
           ...
           /* Request quitGame = Request.parseDelimitedFrom(in);
            if (quitGame.getOperationType() == Request.OperationType.QUIT) {

                writeToLog(name, Message.START);
                System.out.println("Okay " + name);
                Response quit = Response.newBuilder()
                        .setResponseType(Response.ResponseType.BYE)
                        .setMessage("Thanks for Playing. GoodBye!!")
                        .build();
                quit.writeDelimitedTo(out);
