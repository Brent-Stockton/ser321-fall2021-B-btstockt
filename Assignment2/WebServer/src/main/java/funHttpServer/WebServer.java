...
class WebServer {
   ...
   */
  public <exception extends Throwable> byte[] createResponse(InputStream inStream) throws exception {
    ...
    try {
      ...
      if (request == null) {
        ...
        if (request.length() == 0) {
        ...
        else if (request.equalsIgnoreCase("manbearpig")) {
          // opens the random image page

          // open the index.html
          File file = new File("www/manbearpig.html");
          ...
          query_pairs = splitQuery(request.replace("multiply?", ""));
            ...
            // a response that makes sense
              ...
              builder.append("Content-Type: text/html; charset=utf-8\n");
              builder.append("\n");
              builder.append("Please use integers for num1 and/or num2...");

            }
        }
        else if (request.contains("subtract?")) {
          ...
          Map<String, String> query_pairs = new LinkedHashMap<String, String>();
          // extract path parameters
          query_pairs = splitQuery(request.replace("subtract?", ""));
...
/*
          ...
          System.out.println(newObject.getJSONObject("owner").getString("login"));
          System.out.println(newObject.getString("id"));
          JSONArray ownerArray = newObject.getJSONArray("login");
          System.out.println("ownerArray");

          try{
             FileWriter writer = new FileWriter("names.html");

             writer.write(ownerArray.toString());
             writer.flush();
             writer.close();
          } catch (Exception e){
            e.printStackTrace();
